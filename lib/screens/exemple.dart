...
import 'package:emol/utils/MapService.dart';
...
void initState() {
  ...
  WidgetsBinding.instance.addPostFrameCallback((_) {
    ...
    _fetchAgentPositions(); // Ajouté ici pour charger les agents
  });
}
  ...
  for (var service in _services) {
    final icon = await _getMarkerIcon(Colors.orange); // Icône personnalisée avec couleur orange
  ...
  for (var agentPosition in userPositions) {
    final icon = await _getMarkerIcon(Colors.blue); // Icône personnalisée pour l'agent
    newMarkers.add(Marker(
      markerId: MarkerId(agentPosition['userId']),
      position: LatLng(agentPosition['latitude'], agentPosition['longitude']),
      infoWindow: InfoWindow(title: 'Agent ${agentPosition['userId']}'),
      icon: BitmapDescriptor.fromBytes(icon),
    ));
  }
      ...
      if (data is List) {
        setState(() {
          _markers = data.map((agent) {
            return MapService.createMarkerFromAgentData(agent);
          }).toSet();
        });
      }
    }
  ...
  void _updateAgentPosition(String userId, double latitude, double longitude,
      {List<dynamic>? services, String? name}) {
